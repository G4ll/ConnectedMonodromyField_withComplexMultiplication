...
// Step 4: get generators for the simple algebras
    ...
    for block in blocks do
        block_algebra := MatrixAlgebra< QQ, Nrows(block[1]) | block >;
...
// Step 5: recognize each simple component as a field Ei and appoint a generator iota(Ei.1)
    ...
    for block in blocks do
        ...
        iotaE1_big := ZeroMatrix(QQ, Nrows(iotaE_gens[1]));
...
function AlgebraicMatrix(complex_matrix, closure_CMfield, emb_FC)
    ...
    for x in entries do
        if IsCoercible(Q, x) then
            Append(~polys, <x, MinimalPolynomial(QQ!x), QQ!x>);
        else
...
// Step 5: recognize each simple component as a field Ei and appoint a generator iota(Ei.1)
    ...
    block_starting_index := 1;
    for block in blocks do
        iotaE1_big := ZeroMatrix(QQ, Nrows(Qgens[1]));
        InsertBlock(~iotaE1_big, iotaE1, block_starting_index, block_starting_index);
        iotaE1_big := BCM^-1*iotaE1_big*BCM;
...
function ComputePeriod(PP, If)
    ...
    return MinimalPolynomialExtra(period, NumberFieldExtra(DefiningPolynomial(K))), period;
end function;
...
for f in MTequations do
    "------------------";
    //f := MTequations[4]; //Questa equazione dovrebbe proprio avere periodo in Estar
...
"The compositum k.E* is contained in F:", IsSubfield(K, F);
"The field k is contained in E*:", IsSubfield(K, Estar);
